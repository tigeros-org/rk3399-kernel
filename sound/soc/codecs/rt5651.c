...
void power_on_spk(int enable)
{
	
	if (enable){
		int i = 0;
		msleep(100);
		for(i=0; i<SOUND_GRADE; i++){
			udelay(10);
			gpio_set_value(spk_ctl_gpio,!enable);
			udelay(10);
			gpio_set_value(spk_ctl_gpio,!!enable);
		}
	}else{
...
//	printk("1111111111  spk_ctl_status   =%d\n",spk_ctl_status);
	if(spk_ctl_gpio >= 0)
	{
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (spk_ctl_gpio < 0) {
		...
		spk_ctl_status = 0;
		power_on_spk(!spk_active_level);
//	printk("2222222222  spk_active_level   =%d\n",spk_active_level);
	}
