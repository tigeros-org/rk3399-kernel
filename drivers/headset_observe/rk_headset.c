...
#define OPEN_NUM 1
int power_spk(int enabe)
{
	int ret = 0, i = 0;
	int gpio_num = 0;
	
	struct rk_headset_pdata *pdata = headset_info->pdata;
	gpio_num = pdata->spkset_gpio;
	
	ret = gpio_request(gpio_num, "spk_able");
	if(ret < 0){
		printk("%s() devm_gpio_request spkset_gpio request ERROR\n", __FUNCTION__);
		return -1;
	}
	if (enabe)
		{
...
EXPORT_SYMBOL(power_spk);

int headset_flag = 0;
EXPORT_SYMBOL(headset_flag);
