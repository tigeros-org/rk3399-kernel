...
static int rockchip_headset_probe(struct platform_device *pdev)
{
	...
	//spk_ctl_io
	ret = of_get_named_gpio_flags(node, "spk-con-gpio", 0, &flags);
	if (ret < 0) {
		printk("%s() Can not read property spk_gpio\n", __FUNCTION__);
		goto err;
	} else {
		pdata->spkset_gpio = ret;
		pdata->spk_gpio_state = (flags & 1 ) ? 1:0;
	}
	//headset
	...
	if (ret < 0) {
		...
		old_status = gpio_get_value(pdata->headset_gpio);
		if(old_status < 0){
			printk("%s() get head status ERROR\n", __FUNCTION__);
			goto err;
		}
		...
		ret = gpio_request(pdata->spkset_gpio, "spk_able");
		if(ret < 0){
			printk("%s() close spk ERROR\n", __FUNCTION__);
			goto err;
		}
		gpio_direction_output(pdata->spkset_gpio , (pdata->spk_gpio_state & 1)?0:1);
		gpio_free(pdata->spkset_gpio);
